import numpy as np
def gwo_optimize(fitness_func, wolf_population, max_iteration, solution_dimention, lb=-10, ub=10):
    ...
    for t in range(max_iteration):
        ...
        for i in range(wolf_population):
            for j in range(solution_dimention):
                ...
                A1 = 2 * a * r1 - a
                C1 = 2 * r2
                D_alpha = abs(C1 * alpha_positon[j] - positions[i][j])
                X1 = alpha_positon[j] - A1 * D_alpha

                r1, r2 = np.random.rand(), np.random.rand()
                A2 = 2 * a * r1 - a
                C2 = 2 * r2
                D_beta = abs(C2 * beta_positon[j] - positions[i][j])
                X2 = beta_positon[j] - A2 * D_beta

                r1, r2 = np.random.rand(), np.random.rand()
                A3 = 2 * a * r1 - a
                C3 = 2 * r2
                D_delta = abs(C3 * delta_positon[j] - positions[i][j])
                X3 = delta_positon[j] - A3 * D_delta
